...
class Oraculo(val dia: Int, val mes: Int) {
    ...
    private fun obtenerDiaSemana(): Int {
        val calendar = Calendar.getInstance()
        return calendar.get(Calendar.DAY_OF_WEEK)
    }

    // Método para obtener la posición del signo del Zodiaco
    private fun getSignoZodiago(signo: String?): Int {
    ...
    fun obtenerPrediccion(nombre: String?): String {
        var prediccion = "Hola $nombre "
        val numDiaSemana = obtenerDiaSemana() - 1 // Hay que restar 1
        when (signo) {
