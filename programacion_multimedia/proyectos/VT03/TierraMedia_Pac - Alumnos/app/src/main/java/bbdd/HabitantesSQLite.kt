...
class HabitantesSQLite(context: Context) :
    ...
    fun insertarHabitante(habitante: HabitanteTierraMedia): Long {
        ...
        val newRowId = db.insert("habitantes", null, values)
        db.close() // Cerramos conexion a la BD
        return newRowId
    }
    ...
    fun borrarHabitante(idHabitante: Int): Int {
        ...
        db.close() // Cerramos conexion con la BD
        return affectedRows
    }
    ...
    fun getNumeroHabitantesPorRaza(raza: String): Int {
        ...
        val selectQuery = "SELECT count(*) AS numHabitantes FROM habitantes WHERE raza = ?"
        val cursor = db.rawQuery(selectQuery, arrayOf(raza))
        var numHabitantes = 0
        cursor.use {
            if (it.moveToFirst()) {
                numHabitantes = it.getInt(it.getColumnIndexOrThrow("numHabitantes"))
            }
        }
        db.close()
    ...
    fun getListadoPorProfesion(profesion: String): List<HabitanteTierraMedia> {
        ...
        val listaHabitantes = mutableListOf<HabitanteTierraMedia>()
        val db = readableDatabase
        val selectQuery = "SELECT * FROM habitantes ORDER BY dni DESC"
